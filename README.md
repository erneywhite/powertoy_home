...
Откройте **PowerShell** и запустите одну из команд:
...
> ℹ️ Права администратора **не обязательны**. Программы для пользователя ставятся как есть, а для требующих прав (`.msi`, драйверы) запрос UAC появится точечно — только на нужный установщик. Если планируете ставить много системных программ, удобнее сразу запустить PowerShell от имени администратора (тогда отдельных запросов UAC не будет).
...
```
1. Проверка прав администратора (необязательно)
   └─ Без админа скрипт работает; UAC запрашивается точечно только для
      программ, которым нужны права (MSI, драйверы, поле RequiresAdmin)
2. Приветственный экран с ASCII-енотом на 3 секунды
3. Проверка 7-Zip и winget → предложит автоустановку, если их нет
4. Загрузка списка программ из programs.json
5. Меню по категориям с отметкой ✓ для уже установленных
6. Выбор и установка
   ├─ Множественный выбор: «1 5 17» — покажет список и спросит подтверждение, ставит по очереди
...
| 14 | Telegram | .exe | td.telegram.org |
...
├── programs.json              # Список программ (URL, аргументы, опц. WingetId/RequiresAdmin)
├── powertoy.php               # Отдаёт powertoy.ps1 сырьём или HTML-превью; срезает BOM
├── favicon.ico                # Иконка сайта
├── .github/workflows/ci.yml   # CI: JSON-валидация, PSScriptAnalyzer, еженедельный link-check
├── LICENSE                    # GPL-3.0
└── .gitignore
```
...
`powertoy.ps1` при запуске загружает `programs.json` с сервера через `Invoke-WebRequest` (сырые байты + ручное UTF-8-декодирование со срезом BOM — иначе PS 5.1 ломает кириллицу) — добавление/обновление программ не требует правки самого скрипта.

`.github/workflows/ci.yml` — GitHub Actions: на каждый push/PR валидирует `programs.json` и прогоняет PSScriptAnalyzer; раз в неделю проверяет живость всех ссылок и заводит issue, если какая-то перестала отвечать.
...
- `WingetId` (опционально) — идентификатор пакета в [winget](https://learn.microsoft.com/windows/package-manager/winget/), например `"Mozilla.Firefox"`. Если поле задано и `winget` доступен, скрипт сначала пробует установку через него — это даёт последнюю версию и проверенные хеши. При ошибке winget — фолбэк на `Url`.
- `RequiresAdmin` (опционально) — `true`, если для установки нужны права администратора. Тогда (а также для всех `.msi`) скрипт запустит установщик с запросом UAC точечно. Без флага обычные `.exe`, требующие админа, как правило сами показывают UAC.
...
> [!NOTE]
>
> - **Права администратора**: НЕ обязательны. Скрипт работает без админа; программы для пользователя ставятся как есть, а для требующих прав (все `.msi`, драйверы, пункты с `RequiresAdmin`) запрашивается UAC точечно — только на этот установщик.
> - **7-Zip**: необходим для установки программ с пометкой `archive`. При отсутствии скрипт предложит его установить.
> - **BITS**: загрузка через `Start-BitsTransfer -Asynchronous` с прогресс-баром (скорость + ETA) и автоповтором до 3 попыток при сбое — надёжно для больших файлов и нестабильной сети.
...
> - **Winget**: если в `programs.json` задано поле `WingetId` и команда `winget` доступна — скрипт сначала пробует установку через winget, при ошибке переходит на прямую загрузку. Если winget не найден при старте — предложит установить его (App Installer; ставится для пользователя, без админа).
