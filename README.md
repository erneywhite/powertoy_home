...
├── programs.json              # Список программ (URL, аргументы, опц. WingetId/Sha256)
...
- `Sha256` (опционально) — ожидаемый SHA256-хеш скачанного файла. Если задан, скрипт сверяет хеш после загрузки (и при использовании кеша) и **прерывает установку, удаляя файл**, при несовпадении. Полезно для пакетов с собственного хоста. Вычислить: `Get-FileHash файл -Algorithm SHA256`.
...
> - **Проверка целостности (SHA256)**: если у программы задан `Sha256`, скачанный файл сверяется по хешу; при несовпадении файл удаляется и установка прерывается. Без поля проверка пропускается.
